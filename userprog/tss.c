...
void update_tss_esp(struct task_struct* thread) {
  tss.esp0 = (uint32_t*)((uint32_t)thread + PAGE_SIZE);
}
...
static struct gdt_desc make_gdt_desc(uint32_t* desc_addr, uint32_t limit,
  ...
  desc.limit_low_word = (uint16_t)limit;
  desc.base_low_word = (uint16_t)desc_base;
  desc.base_mid_byte = (uint8_t)((desc_base & 0x00ff0000) >> 16);
  desc.attr_low_byte = attr_low;
  desc.limit_high_attr_high = (uint8_t)(((limit & 0x000f0000) >> 16) + attr_high);
  desc.base_high_byte = (uint16_t)(desc_base >> 24);
  return desc;
}
